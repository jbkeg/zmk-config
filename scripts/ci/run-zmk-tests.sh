...
# Based on the upstream zmk-actions test runner with the same behavior.

if [ -z "$1" ]; then
    echo "Usage: ./run-zmk-tests.sh <path to testcase>"
    exit 1
fi
...
if [ "$num_cases" -gt 1 ] || [ "$testcases" != "$path" ]; then
    : >"${ZMK_BUILD_DIR}/tests/pass-fail.log"
...
sh -c "$build_cmd" >"$build_log_tmp" 2>&1
build_exit_code=$?
...
if ! diff -auZ "$path/keycode_events.snapshot" "${ZMK_BUILD_DIR}/tests/${testcase}/keycode_events.log"; then
